        ...
        Statement = [
            {
                ...
                Condition = {
                    StringEquals = {
                        "${data.terraform_remote_state.eks.outputs.aws_iam_openid_connect_provider_extract_arn}:sub": "system:serviceaccount:kube-system:ebs-csi-controller-sa"
                    }
                }
            },
        ]
...
resource "aws_iam_policy_attachment" "ebs_csi_iam_role_policy_attach" {
    policy_arn = aws_iam_policy.ebs_csi_iam_policy.arn
    name = "{local.name-EBS_CSI_IAM}"
    roles = [ aws_iam_role.ebs_csi_iam_role.name ]
}
